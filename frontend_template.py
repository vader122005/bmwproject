...
if st.session_state.messages:
    for msg_idx, msg in enumerate(st.session_state.messages):
        if msg["role"] == "user":
            ...
            if msg.get("error"):
                err = msg["error"]
                if "No data uploaded" in err:
                    st.info(f"📂 {err}")
                elif "Cannot answer" in err or "cannot_answer" in err.lower() or "cannot be answered" in err:
                    # Clarification request — show nicely
                    st.markdown(f"""
                    <div style="background:#fefce8;border:1px solid #fde68a;border-radius:12px;
                        ...
                        <div style="color:#78350f;font-size:0.9rem;line-height:1.6;">{err}</div>
                        <div style="color:#a16207;font-size:0.82rem;margin-top:0.5rem;">
                            💡 Try being more specific — e.g. mention a column name, a year, or a category.
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                elif "does not exist" in err or "incorrect column" in err or "validation failed" in err or "AI used" in err:
                    st.warning(f"⚠️ **Hallucination blocked** — {err}")
                elif "returned no data" in err or "no results" in err.lower() or "no data" in err.lower():
                    st.markdown(f"""
                    <div style="background:#f1f5f9;border:1px solid #e2e8f0;border-radius:12px;
                                padding:1rem 1.2rem;margin:0.5rem 0;">
                        <div style="font-weight:600;color:#475569;margin-bottom:0.3rem;">📭 No matching data found</div>
                        <div style="color:#64748b;font-size:0.88rem;">{err}</div>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.error(f"❌ {err}")
                    ...
                    with st.expander("📋 View data"):
                        if data:
                            ...
                            st.download_button("⬇️ Download CSV", csv_bytes,
                                               ...
                                               mime="text/csv",
                                               key=f"dl_{msg_idx}_{render_idx}")

            if len(charts) == 1:
                render(charts[0], render_idx=0)
            else:
                chart_idx = 0
                for i in range(0, len(charts), 2):
                    ...
                    for ch, col in zip(pair, cols):
                        with col:
                            render(ch, render_idx=chart_idx)
                            chart_idx += 1
            st.write("")
