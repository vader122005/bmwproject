...
def get_schema(db_path):
    ...
    for t in tables:
        ...
        # For text columns, show unique values (up to 20) so LLM knows exact values to filter on
        unique_vals = []
        for col_info in cols:
            col_name = col_info[1]
            col_type = col_info[2].upper()
            if "TEXT" in col_type or "VARCHAR" in col_type or "CHAR" in col_type or col_type == "":
                try:
                    cur.execute(f"SELECT DISTINCT TRIM({col_name}) FROM {t} WHERE {col_name} IS NOT NULL LIMIT 20")
                    vals = [str(r[0]) for r in cur.fetchall() if r[0] is not None]
                    if vals and len(vals) <= 20:
                        unique_vals.append(f"    {col_name} unique values: {', '.join(repr(v) for v in vals)}")
                except Exception:
                    pass

        part = f"Table: {t} ({n} rows)\n  Columns: {col_defs}\n  Sample rows: {samples}"
        if unique_vals:
            part += "\n  Distinct text values (use these EXACTLY in WHERE clauses):\n" + "\n".join(unique_vals)
        parts.append(part)
    conn.close()
    return "\n\n".join(parts)
...
def build_strict_prompt(schema, table_cols, question):
    ...
    return (
        ...
        "7. CRITICAL for text filters: Always use TRIM() and LIKE with % wildcards, never exact = for text. "
        "Example: WHERE TRIM(model) LIKE '%3 Series%' not WHERE model = '3 Series'. "
        "The distinct values shown in the schema are the actual values — match them with LIKE.\n"
        "8. For numeric columns like year, exact = is fine: WHERE year = 2018.\n"
        "9. VAGUE QUERIES: If the user asks something vague or open-ended with no clear analytical intent "
        "(e.g. 'show me something cool', 'give me insights', 'what can you tell me', 'surprise me', "
        "'analyze this', 'show me anything', 'tell me something interesting'), you MUST set cannot_answer=true. "
        "In cannot_answer_reason, respond in plain English explaining that the question is too vague, "
        "and give 2-3 specific example questions the user could ask based on the actual columns available.\n"
        "10. AMBIGUOUS QUERIES: If the query is too ambiguous to answer accurately, set cannot_answer=true and "
        "explain in cannot_answer_reason what specific information the user should provide.\n\n"
        + col_block + "\n"
